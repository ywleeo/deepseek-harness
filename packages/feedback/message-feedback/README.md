...
- **Durable deletion cascades on `session/deleted` only** — `session/disposed`/`host/session-removed` mean detach rather than durable deletion, so the service does not delete valid feedback on detach and may retain empty rows after detach. A permanent `workspace.deleteSession` emits `session/deleted`, which removes the Session row; out-of-band log removal still leaves orphan rows because only the in-product deletion path cascades.
