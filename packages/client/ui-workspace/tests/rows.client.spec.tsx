...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    ...
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} flat t={t} />)
  ...
  it('renders and opens a selected running Session row', () => {
    ...
    render(
      <SessionNodeItem node={node} currentId={node.id} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} t={t} />,
    )
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, close, and delete', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onClose={onClose} onDelete={onDelete} t={t} />)
    ...
    // Delete IS destructive (terminal): danger styling, dispatched to the browser.
    ...
    // Close dispatches without opening the session (and is not destructive).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    expect(screen.getByRole('menuitem', { name: '关闭会话' }).className).not.toMatch(/danger/)
    fireEvent.click(screen.getByRole('menuitem', { name: '关闭会话' }))
    expect(onClose).toHaveBeenCalledWith(node.id)
  ...
  it.each([
    ...
    try {
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} t={t} />)
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    ...
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onClose={vi.fn()} onDelete={vi.fn()} drag={after} t={t} />,
    )
