...
- `ctx.workspaceRegistry.deleteSession(id)` — permanently deletes one session: its persisted log (via the injected `sessionPersistence.delete`), its accounting slot from every workspace, and its archive-set membership, then emits `session/deleted`. The session must exist (live or persisted); a **live** session is refused with `WorkspaceSessionLiveError` because the host cannot dispose a live agent, and an unknown id rejects with `WorkspaceUnknownSessionError`. Deletion is terminal — there is no trash or unarchive.
