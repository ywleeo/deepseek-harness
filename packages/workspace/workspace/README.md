...
- `ctx.workspaceRegistry.deleteSession(id)` — permanently deletes one session: its persisted log (via the injected `sessionPersistence.delete`), its accounting slot from every workspace, and its archive-set membership, then emits `session/deleted`. The session must exist (live or persisted); a still-live session is refused with `WorkspaceSessionLiveError` — disposing an agent is the agent layer's job, so callers (the web gateway's `workspace.deleteSession`) close the session first, and only a re-resumed race reaches the refusal — and an unknown id rejects with `WorkspaceUnknownSessionError`. Deletion is terminal — there is no trash or unarchive.
