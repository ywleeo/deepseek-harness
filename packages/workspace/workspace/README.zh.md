...
- `ctx.workspaceRegistry.deleteSession(id)`：永久删除一个会话：其持久化日志（经注入的 `sessionPersistence.delete`）、其在每个 workspace 中的记账席位及其归档集合成员，随后发出 `session/deleted`。会话必须存在（实时或已持久化）；**实时**会话被拒绝并抛 `WorkspaceSessionLiveError`（宿主无法处置实时 agent），未知 id 抛 `WorkspaceUnknownSessionError`。删除是终局性的——没有回收站，也不能取消归档。
