...
- **Deletion removes the session directory, never automatic retention** — `deleteStored` removes `<root>/<project>/<session-id>/` on request (through the seam's `delete`), but logs otherwise accumulate under `root` until pruned; automatic retention by age/size remains out-of-band backend maintenance.
